...
<body>
    ...
    pauseAutomation() {
        ...
        if (statusText) statusText.textContent = 'Modo Pausado';
        if (statusIndicator) {
            statusIndicator.className = 'status-indicator manual';
            statusIndicator.innerHTML = '<i class="fas fa-pause"></i> <span id="automationStatusText">Modo Pausado</span>';
        }
        
        console.log('⏸️ Automação pausada');
        this.showNotification('Automação pausada. Os leads não avançarão automaticamente.', 'warning');
    }
    ...
    resumeAutomation() {
        ...
        if (statusText) statusText.textContent = 'Modo Automático';
        if (statusIndicator) {
            statusIndicator.className = 'status-indicator auto';
            statusIndicator.innerHTML = '<i class="fas fa-robot"></i> <span id="automationStatusText">Modo Automático</span>';
        }
        
        // Reiniciar automação para leads elegíveis
        this.restartAutomationForEligibleLeads();
        
        console.log('▶️ Automação retomada');
        this.showNotification('Automação retomada. Os leads voltarão a avançar automaticamente.', 'success');
    }
    
    restartAutomationForEligibleLeads() {
        ...
        leads.forEach(lead => {
            if (lead.etapa_atual < 16) { // Não está na última etapa
                ...
                if (timeSinceUpdate >= twoHours) {
                    // Lead elegível para avançar imediatamente
                    this.scheduleLeadAdvancement(lead, 1000); // 1 segundo
                } else {
                    // Agendar para quando completar 2 horas
                    const remainingTime = twoHours - timeSinceUpdate;
                    this.scheduleLeadAdvancement(lead, remainingTime);
                }
            }
        });
    }
    
    scheduleLeadAdvancement(lead, delay) {
        if (this.automationPaused) return;
        
        const timer = setTimeout(() => {
            if (!this.automationPaused && lead.etapa_atual < 16) {
                this.advanceLeadStage(lead.cpf);
                console.log(`🤖 Lead ${lead.nome_completo} avançou automaticamente para etapa ${lead.etapa_atual + 1}`);
            }
        }, delay);
    ...
    showNotification(message, type = 'info') {
        ...
        switch (type) {
            ...
            case 'error':
                ...
                break;
            default:
                notification.style.background = '#345C7A';
        }
