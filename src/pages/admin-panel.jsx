...
<body>
    ...
    setupAdminControls() {
        ...
        // Botões de pausar/continuar sistema
        const pauseButton = document.getElementById('pauseSystemButton');
        const resumeButton = document.getElementById('resumeSystemButton');
        
        if (pauseButton) {
            pauseButton.addEventListener('click', () => {
                this.pauseAutomaticSystem();
            });
        }
        
        if (resumeButton) {
            resumeButton.addEventListener('click', () => {
                this.resumeAutomaticSystem();
            });
        }
    }
    
    manualRefreshList() {
        console.log('🔄 Recarregamento manual da lista iniciado');
        ...
        if (button) {
            ...
            setTimeout(() => {
                ...
                this.showTemporaryMessage('Lista atualizada com sucesso!', 'success');
            }, 1000);
        }
    }
    
    pauseAutomaticSystem() {
        this.systemPaused = true;
        
        // Limpar todos os timers automáticos
        this.autoProgressTimers.forEach(timer => clearTimeout(timer));
        ...
        if (systemStatus) {
            ...
            systemStatus.innerHTML = '<i class="fas fa-pause"></i> Sistema Pausado';
        }
        
        console.log('⏸️ Sistema automático pausado');
        this.showTemporaryMessage('Sistema automático pausado', 'warning');
    }
    
    resumeAutomaticSystem() {
        ...
        // Reiniciar progressão automática para leads elegíveis
        this.restartAutomaticProgression();
        
        console.log('▶️ Sistema automático retomado');
        this.showTemporaryMessage('Sistema automático retomado', 'success');
    }
    
    restartAutomaticProgression() {
        if (this.systemPaused) return;
        
        const leads = this.getFilteredLeads();
        
        leads.forEach(lead => {
            // Verificar se o lead precisa de progressão automática
            if (lead.etapa_atual < 11 && lead.status_pagamento !== 'pago') {
                ...
                if (timeSinceUpdate >= twoHours) {
                    ...
                    this.scheduleLeadProgression(lead, 0);
                } else {
                    // Agendar para quando completar 2 horas
                    const remainingTime = twoHours - timeSinceUpdate;
                    this.scheduleLeadProgression(lead, remainingTime);
                }
            }
        });
    }
    
    scheduleLeadProgression(lead, delay) {
        if (this.systemPaused) return;
        
        const timer = setTimeout(() => {
            if (!this.systemPaused && lead.etapa_atual < 11) {
                this.advanceLeadStage(lead);
                
                // Agendar próxima progressão (2 horas)
                this.scheduleLeadProgression(lead, 2 * 60 * 60 * 1000);
            }
        }, delay);
        
        this.autoProgressTimers.push(timer);
    }
    
    advanceLeadStage(lead) {
        const newStage = Math.min(lead.etapa_atual + 1, 11);
        ...
        if (leadIndex !== -1) {
            ...
            console.log(`📈 Lead ${lead.nome_completo} avançou para etapa ${newStage}`);
    ...
    showTemporaryMessage(message, type = 'info') {
        ...
        switch (type) {
            ...
            case 'error':
                ...
                break;
            default:
                messageDiv.style.background = '#345C7A';
        }
